      ...
      >
        ...
        >
          ...
          >
            ...
            <div v-show="!isEdit">
              {{$filters.getEnumLabel(OUTGOING_TYPE_LIST, customerAccountingPeriod?.billingDate)}}
            </div>
          </el-form-item>
          <el-form-item
            label="出账日设置："
...
import { IOptions } from '@/types/utils';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const customerAccountingPeriod = ref({
      billingDate: '', // 出账类型
      periodDate: '', // 出账日设置
      ...
      isOpenYizubaoAccount: '',
      yizubaoAccountNo: '',
      averagePeriod: '', // 平均账期
    } as IDetailAccountingPeriodCustomerAccountingPeriod);
    ...
    const handleSave = async () => {
      ...
      // 回款周期比例校验
      // let totalPercent = customerAccountingPeriod.value.advancePaymentRatio || 0;
      // customerSettlementPeriodList.value.forEach((item) => {
      //   totalPercent += item.returnPercent;
      // });
      // if (totalPercent !== 100) {
      //   ElMessage.error(
      //     customerAccountingPeriod.value.isAdvancePayment
      //       ? '预付定金百分比+回款比例之和必须等于100%！'
      //       : '回款比例之和必须等于100%！',
      //   );
      //   return;
      // }
