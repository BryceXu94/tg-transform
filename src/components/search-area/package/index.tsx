import { defineComponent, PropType, ref, provide, withKeys, nextTick, onBeforeUpdate, watch } from 'vue';
      ...
      props.config.forEach((item) => {
        if (item.component === 'datePicker') {
          ...
          if (props.modelValue[item?.valueName?.[0] ?? '']) {
            range = [props.modelValue[item?.valueName?.[0] ?? ''], props.modelValue[item?.valueName?.[1] ?? '']];
          }
  ...
  render() {
    ...
    return (
      <el-form
        label-width="96px"
        onKeyup={
          withKeys(() => this.$emit('handleSearch'), ['enter'])
        }
            ...
            this.config
              ...
              .map((item: IConfigItem, index: number) => {
                return (
                  ...
                  >
                    <el-form-item label={item.name} label-width={`${item.component === 'keySelect' ? '0px' : '96px'}`}>
                      {
                        item.slotName ? (
                          <>
